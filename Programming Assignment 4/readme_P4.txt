...
The following functions were written to solve programming assignment 4, with some functions being taken from 
programming assignments 1,2,3 (have been mentioned). 
...
***************************************************************************************************************************
********************************NEW FUNCTIONS FOR USE IN  PROGRAMMING ASSIGNMENT 4*****************************************
***************************************************************************************************************************

ICP_simple.m

This function finds the closest point on a given surface mesh, using a simple iterative closest point algorithm. 
The number of iterations is dependent on the distance between the sample point and the point on the mesh, with an upper 
limit on the number of iterations being defined. 
It computes the sample point 'sk' corresponding to the tip coordinates 'dk', and computes the closest point to 'sk' on 
the surface mesh. Based on this closest point, it recomputes the registration transformation F_reg, and uses this new 
transformation to compute a new sample point 'sk', and iterates. 

Inputs: location of the mesh file 'meshfile', the tip coordinates in every frame 'dk'
Outputs: the sample point 'sk', the closest point on the surface mesh to the tip coordinates in every frame 'c', 
the distance between the two coordinates 'd'. 
...
It computes the sample point 'sk' corresponding to the tip coordinates 'dk', and computes the closest point to 'sk' on 
the surface mesh. However, as opposed to using all the triangles in the mesh, it only uses those whose centroid falls in
the same octant as the sample point 'sk'. 
Based on this closest point, it recomputes the registration transformation F_reg, and uses this new 
transformation to compute a new sample point 'sk', and iterates.
...
this function is used to read any output files if they exist, and return the sample points, the coordinates of the 
points on the surface closest to the sample points, and the magnitude of the difference between the two, as provided 
in the given output files.

Input: This function accepts the file path for the output file 'output' if it exists. 
Outputs: It produces sample points 'sk', the coordinates on the surface mesh 'ck', and the distance between the two 
'diff'.
...
file. The rest of the file is sample points computed, the coordinates of the points on the surface closest to the sample 
points, and the magnitude of the difference between the two. 

Inputs: this function accepts the sample points 'sk', the surface mesh coordinates 'ck', the distance 'diff', and 
the name of the output file which is to be created 'output' 
Outputs: NULL


statisticalAnalysis_P4.m

This is a function that produces a stastical analysis of two sets of data available. This is used to compare the data 
generated by my code with the debug-output files, as well as the data generated using the simple and optimized ICP 
algorithms with each other. 

Inputs: this function accepts two sets of sample points generated, two sets of the closest surface mesh points, 
and two sets of the distances between the sample points and the surface mesh points. 
Outputs: It produces the magnitude of the maximum difference observed between the sample points, and between the 
...
on the surface mesh. It then calls the debugoutput_P4 function to read any output files that may be present. 
Then, it calls the write2file_P4 function to write any outputs you may have generated in the specified format.
Finally, it calls the statisticalAnalysis_P4 function to perform any analysis on the results you may have. 
Comment the function out if there is no output file that has been read. THe statisticalAnalysis function is called 
multiple times to test multiple sets of data. 

Input: NULL
Output: NULL


unit_test_projection.m

This function is a unit test function for the projectionOnSegment.m function. For this, the segment end points and
the data point have been hardcoded. It is used to test the correctness of the algorithm being used. 

Input: NULL
Output: NULL


unit_test_pointSetRegistration.m

This function is a unit test function for the pointSetRegistration.m function. For this, the point clouds 'a' and 'b'
are hardcoded. It is used to test the correctness of the algorithm being used. 

Input: NULL
Output: NULL


unit_test_distanceCalculator.m

This function is a unit test function for the distanceCalculator_Barycentric.m function. For this, the triangle 
coordinates 'p' 'q' 'r' and the point 'a' are hardcoded. It is used to test the correctness of the algorithm being used. 
